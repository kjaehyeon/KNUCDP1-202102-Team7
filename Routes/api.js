module.exports = (app, db) => {
    ...
    router.get('/Alert', (req, res) => {
        ...
        if(!ip){
            ...
            const warehouse_name = db.query('SELECT warehouseName FROM Warehouse'
                                        + ' WHERE iotServer = ?', [ip])[0].warehouseName;
            const admin_phone = db.query("SELECT contactNumber FROM Member WHERE type = 'admin'");
            const provider_phone = db.query('SELECT M.contactNumber'
                                    + ' FROM Member M, Provider P, Warehouse W'
                                    + ' WHERE M.memberID = P.memberID'
                                    + ' AND P.warehouseID = W.warehouseID'
                                    + ' AND W.iotServer = ?', [ip]);
            const buyer_phone = db.query('SELECT M.contactNumber'
